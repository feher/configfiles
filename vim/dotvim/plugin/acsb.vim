...
let s:acsb_displayed_query = 0
let s:acsb_displayed_qres = 0
...
let s:acsb_short_filenames = 1  " 1 == show short file names in [ACSB].
let s:acsb_short_filename_len = 30 " The max length of file names in [ACSB]
				" when acsb_short_filenames is 1.
...
noremap <Leader>c1 :ACSBfindsym ref <c-r>=expand("<cword>")<cr>
noremap <Leader>c2 :ACSBfindsym def <c-r>=expand("<cword>")<cr>
noremap <Leader>c3 :ACSBfindsym called <c-r>=expand("<cword>")<cr>
noremap <Leader>c4 :ACSBfindsym caller <c-r>=expand("<cword>")<cr>
noremap <Leader>c5 :ACSBfindsym txt <c-r>=expand("<cword>")<cr>
noremap <Leader>c6 :ACSBfindsym grep <c-r>=expand("<cword>")<cr>
noremap <Leader>c7 :ACSBfindsym file <c-r>=expand("<cword>")<cr>
noremap <Leader>c8 :ACSBfindsym inc <c-r>=expand("<cword>")<cr>
"inoremap <A-Left> <ESC>:call ACSB_stack_jump(-1)<cr>
"inoremap <A-Right> <ESC>:call ACSB_stack_jump(1)<cr>
noremap <Leader>c<Left> :call ACSB_stack_jump(-1)<cr>
noremap <Leader>c<Right> :call ACSB_stack_jump(1)<cr>
map <silent> <Leader>cq :ACSBQueries<cr>
"imap <silent> <C-Q> <ESC>:ACSBQueries<cr>
noremap <Leader>c<Down> :call <SID>ACSB_iterateQResult(1)<cr>
noremap <Leader>c<Up> :call <SID>ACSB_iterateQResult(-1)<cr>
map <silent> <Leader>cp :ACSBStacks<cr>
"imap <silent> <C-P> <ESC>:ACSBStacks<cr>
map <silent> <Leader>cr :!codelight-rebuild-cscopedb --no-files "%:p:h" . cscope-data kscope-data codelight-data<cr>
...
function! <SID>ACSB_populateACSB()
	setlocal modifiable
	...
	if s:acsb_query_cnt == 0
		...
		while i1 <= s:acsb_query_cnt
			...
			let i2 = 1
			while i2 <= s:acsb_qrescnt_{i1}
				if i1 == s:acsb_displayed_query
					if i2 == s:acsb_displayed_qres
						let putcurhere = lnum + 1
					endif
				endif
				let fname = s:acsb_qres_{i1}_{i2}_1
				if s:acsb_short_filenames == 1
					let partlen = min( [ s:acsb_short_filename_len, strlen(fname) ] )
					let fname = strpart(fname,strlen(fname) - partlen,partlen)
				endif
				let line = "	" . fname . " "
...
function! <SID>ACSB_toggleShortFilenames()
	if s:acsb_short_filenames == 0
		let s:acsb_short_filenames = 1
	else
		let s:acsb_short_filenames = 0
	endif
	call s:ACSB_populateACSB()
endfunction
...
function! <SID>ACSB_showBuff(q, qr)
	...
	let s:acsb_displayed_query = a:q
	let s:acsb_displayed_qres = a:qr
	if a:q == 0
		let s:acsb_displayed_query = s:acsb_active_query
		let s:acsb_displayed_qres = s:acsb_active_qres
	endif
	...
	nnoremap <buffer> <silent> f :call <SID>ACSB_toggleShortFilenames()<CR>
	setlocal nomodifiable
	let s:acsb_shown = 1
endfunction
